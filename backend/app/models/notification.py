...
class Notification(Base):
    ...
    related_medication_id = Column(
        UUID(as_uuid=True),
        ForeignKey("medications.medication_id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    related_health_reading_id = Column(
        UUID(as_uuid=True),
        ForeignKey("health_readings.health_reading_id", ondelete="SET NULL"),
        nullable=True,
    )
    related_extracted_data_id = Column(
        UUID(as_uuid=True),
        ForeignKey("extracted_data.extracted_data_id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    related_health_reading = relationship(
        "HealthReading", back_populates="notifications"
    )
    related_extracted_data = relationship(
        "ExtractedData", back_populates="notifications"
    )
...
class AIAnalysisLog(Base):
    ...
    related_health_reading = relationship(
        "HealthReading", back_populates="ai_analysis_logs"
    )
    related_extracted_data = relationship(
        "ExtractedData", back_populates="ai_analysis_logs"
    )
