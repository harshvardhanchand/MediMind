    ...
    def get_medical_relevance_score(self, item_data: Dict[str, Any], item_type: str) -> float:
        ...
        try:
            ...
            if item_type == 'medication':
                ...
                if symptom_date_str:
                    try:
                        symptom_date = datetime.fromisoformat(symptom_date_str.replace('Z', '+00:00')).date()
                        days_old = (self.clock.today() - symptom_date).days
                        if days_old > 30:
                            base_score = max(0.2, 1.0 - (days_old / 365))  # Decay over a year
                    except:
                        pass
                ...
                if result_date_str:
                    try:
                        result_date = datetime.fromisoformat(result_date_str.replace('Z', '+00:00')).date()
                        days_old = (self.clock.today() - result_date).days
                        if days_old > 90:
                            base_score = max(0.3, 1.0 - (days_old / 730))  # Decay over 2 years
                    except:
                        pass
...
class CRUDOperationHandler:
    ...
    async def _determine_affected_contexts(
        ...
        if data_type == 'medication':
            ...
            if any(keyword in symptom_name for keyword in ['nausea', 'dizziness', 'rash', 'fatigue']):
    ...
    async def _get_recent_symptoms(self, user_id: str, days: int = 90) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            symptoms_result = await self.db.execute(
                ...
                ).order_by(Symptom.reported_date.desc()).limit(20)
            )
            ...
            return [
                {
                    ...
                    "date_time": symptom.reported_date.isoformat() if symptom.reported_date else None,
                    "notes": symptom.notes
                }
    ...
    async def _get_recent_lab_results(self, user_id: str, days: int = 180) -> List[Dict[str, Any]]:
        """Get recent lab results for a user using async queries"""
        try:
            cutoff_date = self.clock.now() - timedelta(days=days)
            readings_result = await self.db.execute(
                select(HealthReading).filter(
                    HealthReading.user_id == user_id,
                    HealthReading.reading_date >= cutoff_date
                ).order_by(HealthReading.reading_date.desc()).limit(30)
            )
            readings = readings_result.scalars().all()
            
            return [
                {
                    "reading_type": reading.reading_type.value if reading.reading_type else "Unknown",
                    "numeric_value": float(reading.numeric_value) if reading.numeric_value else None,
                    "text_value": reading.text_value,
                    "unit": reading.unit,
                    "date_time": reading.reading_date.isoformat() if reading.reading_date else None
                }
    ...
    def _determine_reprocessing_strategy(
        ...
        if ('drug_interactions' in affected_contexts and 
            operation_type in ['create', 'delete'] and 
    ...
    async def _execute_reprocessing_strategy(
        ...
        try:
            if strategy == 'full_interaction_analysis':
                return await self._full_interaction_reanalysis(current_profile, item_data)
                ...
                return await self._analyze_side_effect_correlations(current_profile, item_data)
                ...
                return await self._incremental_context_update(current_profile, item_data)
    ...
    async def _full_interaction_reanalysis(
        ...
        for med in active_medications:
            if self.temporal_context.is_medication_active(med):
                relevance = self.temporal_context.get_medical_relevance_score(med, 'medication')
    ...
    async def _update_historical_context(
        ...
        return {
            ...
            'relevance_score': self.temporal_context.get_medical_relevance_score(item_data, 'medication'),
            'context_updated': True
        }
    ...
    async def _build_current_medical_profile(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for med in medications:
                med['relevance_score'] = self.temporal_context.get_medical_relevance_score(med, 'medication')
    ...
    async def _analyze_side_effect_correlations(self, current_profile, item_data):
        ...
        try:
            symptom_name = item_data.get('symptom', item_data.get('description', 'Unknown symptom'))
            symptom_date = item_data.get('date_time', item_data.get('date'))
            
            # Get medications that could cause this symptom
            active_medications = current_profile.get('medications', [])
            recent_medications = [med for med in active_medications 
                                if self.temporal_context.get_medical_relevance_score(med, 'medication') > 0.5]
            ...
            return {'analysis_type': 'side_effect_correlation_analysis', 'error': str(e)}
    ...
    async def _analyze_health_trends(self, current_profile, item_data):
        ...
        try:
            current_reading_type = item_data.get('reading_type', item_data.get('test_type'))
            ...
            for lab in recent_labs:
                if (lab.get('reading_type') == current_reading_type or 
                    lab.get('test_type') == current_reading_type):
            ...
            # Use Gemini for analysis
            gemini_service = get_gemini_service()
            ai_response = await gemini_service.analyze_lab_trends(same_type_readings + [item_data])
...
class SelectiveReprocessingService:
    ...
    async def reprocess_changed_fields(
        ...
        try:
            ...
            for context, changes in grouped_changes.items():
                logger.info(f"Processing context: {context} with {len(changes)} changes")
    ...
    def _group_changes_by_context(self, changed_fields: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
        """
    ...
    def _extract_context_text(
        ...
        try:
            ...
            context_keywords = {
                'medication_analysis': [
                    'medication', 'drug', 'prescription', 'dose', 'dosage', 'mg', 'ml',
                    'tablet', 'capsule', 'daily', 'twice', 'morning', 'evening',
                    'take', 'prescribed', 'rx', 'refill'
                ],
                'lab_analysis': [
                    'lab', 'test', 'result', 'blood', 'urine', 'glucose', 'cholesterol',
                    'hemoglobin', 'white', 'red', 'count', 'level', 'mg/dl', 'mmol/l',
                    'normal', 'abnormal', 'high', 'low', 'range'
                ],
                'clinical_analysis': [
                    'diagnosis', 'symptom', 'condition', 'patient', 'history',
                    'physical', 'examination', 'assessment', 'plan', 'treatment'
                ],
                'metadata_analysis': [
                    'date', 'doctor', 'physician', 'clinic', 'hospital', 'patient',
            ...
            if relevant_sentences:
                ...
                logger.info(f"Extracted {len(relevant_sentences)} relevant sentences for context {context}")
                return context_text
    ...
    async def _generate_context_specific_suggestions(
        ...
        """
        try:
            prompt = self._build_focused_prompt(context, changes, context_text, current_content)
    ...
    def _merge_ai_suggestions(
        ...
        """
        try:
            if not isinstance(current_content, dict) or not ai_suggestions.get('suggestions'):
                return current_content
                ...
                if confidence >= high_confidence_threshold:
                    ...
                    if section == 'extracted_metadata':
                        ...
                        if 0 <= index < len(updated_content['medical_events']):
                            updated_content['medical_events'][index][field] = suggested_value
                    
                    logger.info(f"Applied AI suggestion: {section}.{field} = {suggested_value} (confidence: {confidence})")
            ...
            for suggestion in suggestions:
                ...
                if confidence < high_confidence_threshold:
                    ...
                    logger.info(f"Low confidence suggestion not applied: {suggestion.get('section')}.{suggestion.get('field')} - {reason} (confidence: {confidence})")
