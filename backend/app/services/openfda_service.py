...
class OpenFDAService:
    ...
    async def analyze_drug_interactions(
        ...
        try:
            ...
            significant_interactions = self._filter_significant_interactions(
                fda_interactions, patient_context or {}
            )

            # Format final result
            return self._format_interaction_result(
                significant_interactions, patient_context
            )
    ...
    def _normalize_drug_names(self, medications: List[Dict[str, Any]]) -> List[str]:
        ...
        for med in medications:
            ...
            if drug_name:
                ...
                candidates = medical_normalizer.get_comprehensive_drug_candidates(
                    drug_name
                )
    ...
    async def _check_fda_interactions(
    ...
    async def _query_drug_pair(
        self, session: aiohttp.ClientSession, drug1: str, drug2: str
        ...
        try:
            ...
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    logger.warning(
                        f"FDA API returned status {response.status} for {drug1} + {drug2}"
                    )
                    return None
    ...
    def _parse_fda_interaction(
        self, drug1: str, drug2: str, fda_data: Dict
    ) -> Dict[str, Any]:
        """
        ...
        for event in fda_data["results"][:10]:  # Analyze first 10 events
            ...
            for reaction in reactions:
                ...
                if reaction_term:
                    common_reactions[reaction_term] = (
                        common_reactions.get(reaction_term, 0) + 1
                    )
        ...
        return {
            ...
            "common_reactions": dict(
                sorted(common_reactions.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
            "source": "FDA_ADVERSE_EVENTS",
        }

    def _filter_significant_interactions(
        self, interactions: List[Dict[str, Any]], patient_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _calculate_significance_score(
        self, interaction: Dict[str, Any], patient_context: Dict[str, Any]
    ) -> float:
        """
    ...
    def _score_reactions(self, reactions: Dict[str, int]) -> float:
        ...
        for reaction, count in reactions.items():
            ...
            if any(keyword in reaction_lower for keyword in high_severity_keywords):
                ...
                keyword in reaction_lower for keyword in moderate_severity_keywords
            ):
    ...
    def _score_patient_risk(self, patient_context: Dict[str, Any]) -> float:
        ...
        for condition in conditions:
            ...
            if any(
                risk_condition in condition_name
    ...
    def _generate_recommendations(
        ...
        if immediate:
            recommendations.append(
                "IMMEDIATE: Contact your doctor before taking these medications together"
            )

        if high:
            recommendations.append(
                "🔴 HIGH: Discuss these medication combinations with your healthcare provider"
            )

        if len(interactions) >= 3:
            recommendations.append(
                "📋 Consider medication review with pharmacist due to multiple interactions"
            )
    ...
    def _create_error_result(self, error_message: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "medium_priority": [],
            "recommendations": [
                "❌ Unable to check interactions. Please consult your healthcare provider."
            ],
            "confidence": 0.0,
            "error": error_message,
            "analysis_timestamp": datetime.now().isoformat(),
