...
class MedicalAIService:
    ...
    async def analyze_medical_event(
        ...
        try:
            ...
            if (
                ...
                and len(medical_profile["medications"]) >= 2
            ):
                fda_analysis = await self._analyze_drug_interactions_fda(
                    medical_profile
                )

            # 3. Comprehensive Multi-Correlation Analysis
            correlation_analysis = (
                await multi_correlation_analyzer.analyze_comprehensive_correlations(
                    medical_profile, {"type": trigger_type, **event_data}
                )
            )

            # 4. Create medical embedding for similarity search (for cost optimization)
            medical_embedding = medical_embedding_service.create_medical_embedding(
                medical_profile
            )

            # 5. Search for similar situations
            similar_situations = await self.vector_search.find_similar_situations(
                medical_embedding, medical_profile
            )
            ...
            high_priority_correlations = [
                c
                for c in correlation_analysis.get("correlations", [])
            ...
            if not high_priority_correlations and not similar_situations:
                ...
                logger.info(
                    "Low-confidence correlations, calling LLM for additional analysis"
                )
                ...
                if (
                    llm_analysis
                    and llm_analysis.get("confidence", 0) >= self.confidence_threshold
                ):
                ...
                logger.info(
                    f"Found {len(similar_situations)} similar situations, adapting analysis"
                )
                llm_analysis = await self._adapt_similar_analysis(
                    medical_profile, similar_situations[0]  # Use most similar
                )

                # Update usage count for the reused situation
                await self.vector_db.update_usage_count(
                    similar_situations[0]["situation_id"]
                )

            # 7. Merge FDA, Correlation, and LLM analysis results
            merged_analysis = self._merge_all_analysis_results(
                fda_analysis, correlation_analysis, llm_analysis
            )

            # 8. Create notifications from merged analysis
            notifications = await self._create_notifications_from_analysis(
                user_id, merged_analysis, trigger_type, event_data
            )
            ...
            logger.info(
                f"Analysis complete: {len(notifications)} notifications generated"
            )
    ...
    async def _build_medical_profile(
        ...
        try:
            ...
            conditions = (
                self.db.query(HealthCondition)
                .filter(
                    HealthCondition.user_id == user_id,
                    HealthCondition.status == "active",
                )
                .order_by(HealthCondition.diagnosed_date.desc())
                .all()
            )
            ...
            profile["health_conditions"] = [
                {
                    ...
                    "severity": (
                        condition.severity.value if condition.severity else None
                    ),
                }
            ...
            profile["lab_results"] = [
                {
                    ...
                    "value": (
                        float(reading.numeric_value)
                        if reading.numeric_value
                        else reading.text_value
                    ),
                    "unit": reading.unit,
    ...
    async def _analyze_drug_interactions_fda(
        ...
        try:
            ...
            fda_result = await openfda_service.analyze_drug_interactions(
                medications, patient_context
            )

            logger.info(
                f"FDA analysis result: {fda_result.get('total_interactions', 0)} interactions found"
            )
            return fda_result
    ...
    def _merge_all_analysis_results(
        ...
        merged["correlations"] = self._prioritize_and_deduplicate_correlations(
            merged["correlations"]
        )
    ...
    def _calculate_correlation_confidence(
        ...
        top_correlations = sorted(
            correlations, key=lambda x: x.get("confidence", 0), reverse=True
        )[:3]
        avg_confidence = sum(c.get("confidence", 0) for c in top_correlations) / len(
            top_correlations
        )

        return avg_confidence

    def _convert_fda_to_correlation_format(
        self, fda_analysis: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
            ...
            correlations.append(
                {
                    ...
                    "medications": [m.lower() for m in pair],
                    "confidence": 0.95,
                    "severity": "high",
                    "priority_score": 0.95,
    ...
    def _merge_llm_insights(
        self, merged: Dict[str, Any], llm_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _prioritize_and_deduplicate_correlations(
        self, correlations: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """Prioritize and remove duplicate correlations"""
        # Sort by priority score
        sorted_correlations = sorted(
            correlations, key=lambda x: x.get("priority_score", 0), reverse=True
        )
    ...
    def _generate_final_recommendations(
        self, merged_analysis: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _adapt_similar_analysis(
        self, current_profile: Dict[str, Any], similar_situation: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
    ...
    def _create_medical_analysis_prompt(self, medical_profile: Dict[str, Any]) -> str:
        ...
        if medications:
            for med in medications:
                prompt_parts.append(
                    f"- {med.get('name', 'Unknown')} {med.get('dosage', '')} {med.get('frequency', '')}"
                )
        else:
            prompt_parts.append("- None reported")

        prompt_parts.extend(["", "RECENT SYMPTOMS:"])

        symptoms = medical_profile.get("recent_symptoms", [])
        if symptoms:
            for symptom in symptoms:
                prompt_parts.append(
                    f"- {symptom.get('symptom', 'Unknown')} (Severity: {symptom.get('severity', 'Unknown')})"
                )
        else:
            prompt_parts.append("- None reported")

        prompt_parts.extend(["", "HEALTH CONDITIONS:"])
        ...
        prompt_parts.extend(["", "RECENT LAB RESULTS:"])

        labs = medical_profile.get("lab_results", [])
        if labs:
            for lab in labs[-5:]:  # Only show recent 5
                prompt_parts.append(
                    f"- {lab.get('test', 'Unknown')}: {lab.get('value', 'N/A')} {lab.get('unit', '')}"
                )
        else:
    ...
    def _parse_llm_response(self, llm_response: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "drug_interactions": self._extract_section(
                    llm_response, "drug interaction"
                ),
    ...
    def _create_correlation_type_notification(
        ...
        primary_correlation = max(
            correlations, key=lambda x: x.get("priority_score", 0)
        )

        # Determine notification type and severity
        notification_type, severity = self._map_correlation_to_notification_type(
            correlation_type, primary_correlation
        )

        # Generate title and message
        title, message = self._generate_correlation_notification_content(
    ...
    def _map_correlation_to_notification_type(
        self, correlation_type: str, correlation: Dict[str, Any]
    ) -> Tuple[str, str]:
        """Map correlation type to notification type and severity"""
        type_mapping = {
            "drug_symptom_correlation": (
                "side_effect_warning",
                correlation.get("severity", "medium"),
            ),
            "lab_symptom_correlation": ("health_trend", "medium"),
            "drug_lab_correlation": (
                "monitoring_alert",
                correlation.get("concern_level", "medium"),
            ),
            "drug_interaction_fda": (
                "drug_interaction",
    ...
    def _generate_correlation_notification_content(
        self, correlation_type: str, correlations: List[Dict[str, Any]]
    ) -> Tuple[str, str]:
        ...
        if correlation_type == "drug_symptom_correlation":
            ...
            message = primary.get(
                "recommendation",
                "Review this correlation with your healthcare provider.",
            )
        ...
        for alert in risk_alerts[:2]:  # Top 2 risk alerts
            notification = {
                ...
                "related_medication_id": (
                    event_data.get("medication_id") if event_data else None
                ),
                "related_document_id": (
                    event_data.get("document_id") if event_data else None
                ),
                "related_health_reading_id": (
                    event_data.get("health_reading_id") if event_data else None
                ),
                "related_extracted_data_id": (
                    event_data.get("extracted_data_id") if event_data else None
                ),
            }
            notifications.append(notification)
    ...
    async def _log_analysis(
        ...
        try:
            ...
            profile_hash = medical_embedding_service.create_medical_hash(
                medical_profile
            )
            ...
            embedding_str = (
                str(embedding.tolist()) if hasattr(embedding, "tolist") else str([])
            )
            ...
            await self.db.execute(
                ...
                ),
                {
                    ...
                    "similarity_matches": json.dumps(
                        similar_situations[:2]
                    ),  # Store top 2 matches
