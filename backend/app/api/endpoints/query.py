...
from app.utils.ai_processors import (
    extract_query_filters_with_gemini,
    answer_query_with_filtered_context_gemini,
)
...
class ExtractedFilters(BaseModel):
    document_type: Optional[str] = Field(
        None,
        pattern="^(prescription|lab_result|imaging_report|consultation_note|discharge_summary|other)$",
    )
...
async def classify_and_handle_general_query(
    api_key: str, query_text: str
) -> Optional[str]:
    """
...
def resolve_date_range(date_filter: Any) -> Optional[Tuple[date, date]]:
    ...
    if (
        isinstance(date_filter, dict)
        and "start_date" in date_filter
        and "end_date" in date_filter
    ):
        try:
...
async def natural_language_query_endpoint(
    ...
    if not user_id:
        logger.error(
            f"Could not retrieve user_id from token for query: {query_request.query_text}"
        )
        raise HTTPException(status_code=403, detail="User not found or invalid token.")

    logger.info(
        f"Processing query: '{query_request.query_text}' for user_id: {user_id}"
    )

    general_response = await classify_and_handle_general_query(
        api_key=settings.GEMINI_API_KEY, query_text=query_request.query_text
    )
    ...
    try:
        ...
        if filter_json_str is None:
            logger.error(
                f"LLM filter extraction failed for query: {query_request.query_text}"
            )
            raise HTTPException(
                status_code=500, detail="Failed to analyze query for filtering."
            )
        ...
        try:
            ...
            logger.error(
                f"Failed to parse JSON filters from LLM: {filter_json_str}, error: {e}"
            )
            raise HTTPException(
                status_code=500, detail="AI returned malformed filter data"
            )
        except Exception as e:
            logger.error(f"Failed to validate LLM filters: {raw_filters}, error: {e}")
            raise HTTPException(
                status_code=500, detail="AI returned invalid filter format"
            )
        ...
        if extracted_filters.get("date_range"):
            ...
            if resolved_range:
                query_filters["document_date_range"] = (
                    resolved_range  # Use a key the repo understands
                )
        ...
        logger.debug(
            f"Step 3: Preparing context from {len(filtered_documents)} documents and generating answer..."
        )
        ...
        for doc in filtered_documents:
            ...
            if extracted_data and extracted_data.content:
                ...
                if isinstance(medical_events_from_doc, list):
                    ...
                    logger.warning(
                        f"Document {doc.document_id} has content but no 'medical_events' list or it's not a list."
                    )
            else:
                logger.warning(
                    f"No ExtractedData or no content for document_id: {doc.document_id}"
                )

        if not all_medical_events:
            logger.warning(
                f"Filtered documents found, but no extractable medical_events for query: {query_request.query_text}"
            )
        ...
        if len(json_data_context_str) > MAX_CONTEXT_CHARS:
            logger.warning(
                f"Context size ({len(json_data_context_str)} chars) exceeds limit, intelligently truncating"
            )

            truncated_events = all_medical_events
            while (
                len(json.dumps(truncated_events, indent=2)) > MAX_CONTEXT_CHARS
                and truncated_events
            ):
            ...
            logger.info(
                f"Truncated from {len(all_medical_events)} to {len(truncated_events)} medical events"
            )
        ...
        if llm_answer is None:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to get an answer from the AI assistant based on the filtered documents.",
            )

        logger.info(
            f"Successfully generated answer for query: '{query_request.query_text}'"
        )
        ...
        logger.error(
            f"Value error during query processing for '{query_request.query_text}': {e}"
        )
        raise HTTPException(status_code=400, detail="Invalid input data format")
    except ConnectionError as e:
        logger.error(
            f"Connection error during query processing for '{query_request.query_text}': {e}"
        )
        raise HTTPException(status_code=503, detail="External service unavailable")
    except Exception as e:
        logger.error(
            f"Unexpected error during query processing for '{query_request.query_text}': {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
