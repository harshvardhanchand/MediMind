...
async def get_current_user_id_from_token(
    ...
    if not supabase_id:
        raise HTTPException(
            status_code=401, detail="Invalid authentication token: no sub claim"
        )
...
async def get_extracted_data(
    ...
    if doc.user_id != current_user_id:
        raise HTTPException(
            status_code=403,
            detail="Not authorized to access this document's extracted data",
        )

    extracted_data = extracted_data_repo.get_by_document_id(
        db=db, document_id=document_id
    )
    if not extracted_data:
        raise HTTPException(
            status_code=404, detail="Extracted data not found for this document"
        )
...
async def update_extracted_data_status(
    ...
    if doc.user_id != current_user_id:
        raise HTTPException(
            status_code=403,
            detail="Not authorized to update this document's extracted data",
        )
    ...
    if not updated_extracted_data:
        raise HTTPException(
            status_code=404,
            detail="Failed to update extracted data status or data not found",
        )
...
async def update_extracted_data_content(
    ...
    existing_data = extracted_data_repo.get_by_document_id(
        db=db, document_id=document_id
    )
    ...
    try:
        ...
        updated_data = extracted_data_repo.update_structured_content(
            db=db, document_id=document_id, content=content_update.content
        )
        if not updated_data:
            db.rollback()
            raise HTTPException(
                status_code=404,
                detail="Failed to update extracted data content or data not found",
            )
        ...
        if not final_updated_data:
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to update review status after content update",
            )
    ...
    if (
        content_update.trigger_selective_reprocessing
        and content_update.changed_fields
        and len(content_update.changed_fields) > 0
    ):
    ...
    try:
        ...
        if changes:
            ...
            logger.info(
                f"AI analysis triggered for document re-processing: {document_id}"
            )
...
def trigger_auto_population_and_analysis(
    ...
    try:
        ...
        population_result = auto_population_service.populate_from_extracted_data(
            user_id=user_id, extracted_data=extracted_data, document_id=document_id
        )

        logger.info(
            f"Auto-population completed for document {document_id}: {population_result}"
        )
        ...
        if total_created > 0:
            try:
                logger.info(
                    f"Triggering AI analysis for {total_created} auto-populated entries"
                )
                notification_service = get_notification_service(db)
                medical_triggers = get_medical_triggers(notification_service)
                ...
                logger.info(
                    f"AI analysis triggered successfully for document {document_id}"
                )

            except Exception as ai_error:
                logger.error(
                    f"Failed to trigger AI analysis for document {document_id}: {str(ai_error)}"
                )
        else:
            logger.info(
                f"No new entries created for document {document_id}, skipping AI analysis"
            )

    except Exception as e:
        logger.error(
            f"Auto-population and analysis failed for document {document_id}: {str(e)}"
        )
...
def run_selective_reprocessing(
    ...
    try:
        ...
        if enhanced_content != current_content:
            ...
            updated_data = extracted_data_repo.update_structured_content(
                db=db, document_id=document_id, content=enhanced_content
            )

            if updated_data:
                logger.info(
                    f"Successfully applied AI suggestions for document {document_id}"
                )
                ...
                logger.warning(
                    f"Failed to save AI suggestions for document {document_id}"
                )
        else:
            logger.info(f"No AI improvements suggested for document {document_id}")

        logger.info(
            f"Selective reprocessing completed successfully for document {document_id}"
        )

    except Exception as e:
        logger.error(
            f"Selective reprocessing failed for document {document_id}: {e}",
            exc_info=True,
        )
...
async def get_extraction_details(
    ...
    if doc.user_id != current_user_id:
        raise HTTPException(
            status_code=403, detail="Not authorized to access this document's details"
        )
