...
@router.get(
    ...
    summary="Get medications for current user",
)
async def get_medications(
    ...
    search: Optional[str] = Query(
        None, description="Search query for medication name, reason, or doctor"
    ),
    ...
    try:
        ...
        logger.info(
            f"Retrieved {len(medications)} medications for user {current_user.user_id} (optimized={optimized})"
        )
        return medications
...
@router.get(
    "/dashboard",
    response_model=List[MedicationResponse],
    summary="Get medications for dashboard",
)
async def get_medications_for_dashboard(
    limit: int = Query(
        5, le=10, description="Maximum number of medications for dashboard"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
...
async def get_medication(
    ...
    full_details: bool = Query(
        True, description="Load all related data (notifications, documents, etc.)"
    ),
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    ...
    try:
        if full_details:
            medication = medication_repo.get_by_id_with_full_details(
        ...
        if medication.user_id != current_user.user_id:
            logger.warning(
                f"User {current_user.user_id} attempted to access medication {medication_id} owned by {medication.user_id}"
            )
            raise HTTPException(
        ...
        logger.error(
            f"Error retrieving medication {medication_id}: {str(e)}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
@router.put(
    "/{medication_id}", response_model=MedicationResponse, summary="Update a medication"
)
async def update_medication(
    ...
    if db_medication.user_id != current_user.user_id:
        logger.warning(
            f"User {current_user.user_id} attempted to update medication {medication_id} owned by {db_medication.user_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access denied to this medication",
    ...
    try:
        ...
        old_medication_data = {
            ...
            "frequency": (
                db_medication.frequency.value if db_medication.frequency else None
            ),
            "start_date": (
                db_medication.start_date.isoformat()
                if db_medication.start_date
                else None
            ),
            "end_date": (
                db_medication.end_date.isoformat() if db_medication.end_date else None
            ),
        ...
        try:
            ...
            new_medication_data = {
                ...
                "frequency": (
                    updated_medication.frequency.value
                    if updated_medication.frequency
                    else None
                ),
                "start_date": (
                    updated_medication.start_date.isoformat()
                    if updated_medication.start_date
                    else None
                ),
                "end_date": (
                    updated_medication.end_date.isoformat()
                    if updated_medication.end_date
                    else None
                ),
                "status": (
                    updated_medication.status.value
                    if updated_medication.status
                    else None
                ),
            ...
            if changes:
                ...
                logger.info(
                    f"AI analysis triggered for medication update: {medication_id}"
                )
...
async def delete_medication(
    ...
    if db_medication.user_id != current_user.user_id:
        logger.warning(
            f"User {current_user.user_id} attempted to delete medication {medication_id} owned by {db_medication.user_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Access denied to this medication",
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"AI analysis triggered for medication deletion: {medication_id}"
            )
